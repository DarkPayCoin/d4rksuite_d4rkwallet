...
import (
	...
	"github.com/particl/partsuite_partwallet/chain"
	"github.com/particl/partsuite_partwallet/waddrmgr"
	"github.com/particl/partsuite_partwallet/wallet/txauthor"
	"github.com/particl/partsuite_partwallet/wallet/txrules"
	"github.com/particl/partsuite_partwallet/walletdb"
	"github.com/particl/partsuite_partwallet/wtxmgr"
)
...
func (w *Wallet) RelayFee() partutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee partutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
// rescan request.
func (w *Wallet) activeData() ([]partutil.Address, []wtxmgr.Credit, error) {
	var addrs []partutil.Address
	err := w.Manager.ForEachActiveAddress(func(addr partutil.Address) error {
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (partutil.Amount, error) {
...
type Balances struct {
	Total          partutil.Amount
	Spendable      partutil.Amount
	ImmatureReward partutil.Amount
}
...
func (w *Wallet) CurrentAddress(account uint32) (partutil.Address, error) {
...
func ListTransactions(details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal partutil.Amount
		for _, deb := range details.Debits {
			debitTotal += deb.Amount
		}
		var outputTotal partutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += partutil.Amount(output.Value)
		}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := partutil.Amount(output.Value).ToBTC()
	...
	loopDetails:
		for i := range details {
			...
			for _, cred := range detail.Credits {
				...
				apkh, ok := addrs[0].(*partutil.AddressPubKeyHash)
				if !ok {
					continue
				}
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	m := make(map[uint32]*partutil.Amount)
...
func (w *Wallet) DumpWIFPrivateKey(addr partutil.Address) (string, error) {
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *partutil.WIF, bs *waddrmgr.BlockStamp,
	...
	if rescan {
		job := &RescanJob{
			Addrs:      []partutil.Address{addr.Address()},
			OutPoints:  nil,
			BlockStamp: *bs,
		}
...
func (w *Wallet) NewAddress(account uint32) (partutil.Address, error) {
	...
	utilAddrs := make([]partutil.Address, len(addrs))
...
func (w *Wallet) NewChangeAddress(account uint32) (partutil.Address, error) {
...
// account.
func (w *Wallet) TotalReceivedForAccount(account uint32, minConf int32) (partutil.Amount, int32, error) {
	syncBlock := w.Manager.SyncedTo()
...
// returning the total amount of bitcoins received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr partutil.Address, minConf int32) (partutil.Amount, error) {
	syncBlock := w.Manager.SyncedTo()
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*partutil.WIF,
	...
	for i, txIn := range tx.TxIn {
		...
		getKey := txscript.KeyClosure(func(addr partutil.Address) (
		...
		getScript := txscript.ScriptClosure(func(
			addr partutil.Address) ([]byte, error) {
